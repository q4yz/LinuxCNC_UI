...
test("CameraSettings wires a Custom buttons editor for the camera viewer slot", () => {
  const text = read(settingsPath);
  // The new "Custom buttons" section uses the shared
  // ``MacroButtonEditor`` against the camera module's
  // ``SettingsStore``. The slot id is ``camera.bottom`` so the
  // ``CameraViewer``'s ``MacroButton`` lookup finds the matching
  // descriptor. Both ends of the contract are asserted here so
  // a future rename trips the test.
  assert.match(
    text,
    /import\s*\{[^}]*\bMacroButtonEditor\b[^}]*\}\s*from\s*["'][^"']*\/ui["']/,
    "CameraSettings must import MacroButtonEditor from the shared ui barrel",
  );
  assert.match(
    text,
    /<MacroButtonEditor\b/,
    "CameraSettings must render MacroButtonEditor",
  );
  assert.match(
    text,
    /id:\s*["']camera\.bottom["']/,
    "CameraSettings must declare the camera.bottom slot id",
  );
  assert.match(
    text,
    /buttonConfig\.persist/,
    "CameraSettings must persist the editor's v-model back to settings",
  );
});

test("CameraViewer renders a MacroButton for the camera.bottom slot", () => {
  const text = read(viewerPath);
  // The viewer renders ``MacroButton`` for the camera slot so an
  // operator-configurable shortcut (e.g. light-on) can sit next to
  // the existing "Switch Camera" button.
  assert.match(
    text,
    /import\s*\{[^}]*\bMacroButton\b[^}]*\}\s*from\s*["'][^"']*\/ui["']/,
    "CameraViewer must import MacroButton from the shared ui barrel",
  );
  assert.match(
    text,
    /buttonsBySlot\[['"]camera\.bottom['"]\]/,
    "CameraViewer must look up the camera.bottom slot",
  );
  assert.match(
    text,
    /<MacroButton[\s\S]{0,300}camera\.bottom/,
    "CameraViewer must render MacroButton with the camera.bottom descriptor",
  );
});

test("CameraSettings does not deep-watch buttonConfig.buttons.value", () => {
  // Regression guard for the request-spam loop. The host must
  // rely on the editor's ``update:modelValue`` emit alone; a
  // ``watch(() => buttonConfig.buttons.value, ..., { deep: true })``
  // would re-fire ``persist`` on every nested change and
  // produce ~100 PUTs per click.
  const text = read(settingsPath);
  assert.doesNotMatch(
    text,
    /watch\(\s*\(\)\s*=>\s*buttonConfig\.buttons\.value/,
    "CameraSettings must not watch buttonConfig.buttons.value (regression)",
  );
  assert.match(
    text,
    /@update:model-value="\(next\)\s*=>\s*buttonConfig\.persist\(next\)"/,
    "CameraSettings must persist through the editor's update:modelValue emit",
  );
});
