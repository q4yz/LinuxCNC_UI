...
// between the frontend and the backend. The frontend manifest
// (``frontend/src/modules/machine/manifest.ts``) declares
// ``id: "machine"`` (the canonical registry id), but the
// backend's ``_MODULE_DOMAINS[0]`` mounts this module's
// settings router under ``id: "axis"`` — the URL is therefore
...
// ``<data_root>/modules/axis/settings.json`` file. The frontend
// registry still looks the module up under ``"machine"``; the
// settings URL is the only place where ``"axis"`` shows up. A
// future rename that aligns the two sides should pick one name
// and update every reference in lockstep.
