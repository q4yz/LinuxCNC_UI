...
//
// ``config`` is the existing entry for the Config view. The
// machineconfig module reuses this slot so the rail still lands in
// ``ConfigView`` without introducing a second module-only shell.
//
// ``macros`` was added in issue #7 and surfaces the macro editor
// alongside the other built-in entries.
const builtinItems = [
  ...
  { id: 'macros', label: 'Macros', icon: '<svg class="w-6 h-6 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>', order: 100 },
