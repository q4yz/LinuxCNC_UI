...
// The App shell renders the corresponding module's ``mainView`` for
// any sidebar-id-style name (``camera``, ``machineconfig``, ...) via
// the registry-driven ``registerModuleRoutes`` hook. Built-in
// dashboard / programs / settings names map directly to their view.
...
// built-ins (which all default to ``order: 100``). The legacy
// ``config`` entry that used to live here is gone: ``machineconfig``
// owns that sidebar slot now (``manifest.sidebar.id ===
// 'machineconfig'``).
const builtinItems = [
