...
async function downloadMachineFile(entry) {
  const content = await store.readMachineContent(entry.path);
  if (content === null) return;
    ...
    <ul v-if="entries.length" class="flex-1 space-y-1 overflow-y-auto p-2 pb-20">
      <li v-for="entry in entries" :key="entry.path" class="relative">
        ...
        >
          ...
          <button v-if="entry.kind === 'file'" type="button" class="rounded px-2 py-1 text-gray-300 hover:bg-gray-600" title="Download" @click.stop="downloadMachineFile(entry)">↓</button>
