...
// ------------------------------------------------------------------- //
//
// Lives only on JoggingView now — it used to also appear embedded in
// Dashboard's scrolling column, where a live WebGL canvas fighting
...
// Still Teleported rather than mounted directly in JoggingView so a
// single WebGL context/Three.js scene survives route changes instead
// of being torn down and rebuilt — the instance just parks (hidden,
// RAF loop paused) when the operator is elsewhere. ``isToolpathViewActive``
// pauses the render loop while parked instead of rendering an
// invisible canvas.
//
...
const isToolpathViewActive = computed(() => route.name === 'jogging')
const toolpathTeleportTarget = ref('#toolpath-parking')

watch(
  () => route.name,
  () => {
    if (route.name === 'jogging') toolpathTeleportTarget.value = '#toolpath-slot-jogging'
...
<template>
  <div class="flex flex-col h-screen overflow-hidden bg-gray-900 text-white font-sans">
    ...
    <!-- Shared 3D toolpath viewer — a single instance, moved into
         JoggingView's slot via Teleport while that view is active
         (see script setup above). Parked (hidden, RAF loop paused) in
         index.html's #toolpath-parking div — which must live outside
         this component tree, see the comment there — everywhere else. -->
