...
- There is a real, substantial pytest suite (three separate suites — see above) and a frontend `node --test` suite, but no configured linter/type-checker gate for either language yet (see the technical-debt list in [.agent/HANDOFF.md](.agent/HANDOFF.md) § 2). Follow the conventions above by hand until that lands. Run every command in [.agent/TEST.md](.agent/TEST.md) before handoff.
