...
2. ``HardwareJson.model_rebuild(force=True)`` — full schema recompile
   on the v2 model. **Prime suspect** for the SIGILL: forces a
   recompile of the forward-referenced ``mcus`` list, exercises
   pydantic_core's compile path differently than lazy first-use.
