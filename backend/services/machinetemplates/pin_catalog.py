...
def build_pin_catalog(
    ...
    """Collect the catalog from the preloaded services.

    The services return their cached containers (never re-built), so
    calling this repeatedly is side-effect free. Pass the ``*_containers``
    overrides in tests to avoid touching the services entirely.
    """
    if tool_containers is None or sensor_containers is None or state_containers is None:
    ...
    tools = (
        ...
        else get_tools_service().get_halpins()
    )
    sensors = (
        sensor_containers
        if sensor_containers is not None
        else get_temperature_service().get_halpins()
    )
    state = (
        state_containers
        if state_containers is not None
        else get_state_service().get_halpins()
    )
