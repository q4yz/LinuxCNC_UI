...
@router.get("/profiles", summary="List compiler profiles")
def get_profiles():
    """List available configuration profiles for compilation."""
    try:
        return {"status": "ok", "profiles": _list_profile_files()}
...
@router.post("/generate/{profile_name}", summary="Generate staged compiler files")
def generate_staged(profile_name: str):
...
@router.post("/deploy", summary="Deploy staged compiler files")
def deploy_staged():
