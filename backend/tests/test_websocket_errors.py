...
def _reset():
    ...
    """
    linuxcnc_mock.reset_error_history()


def test_push_error_appends_entry():
    _reset()
    linuxcnc_mock.record_error(
        kind=11,
...
def test_get_current_state_returns_a_copy_not_a_live_reference():
    ...
    # ``list(...)`` snapshot in ``read_error_history`` decouples
...
def test_telemetry_loop_pushes_into_history_before_broadcast():
    ...
    # manually invoking ``record_error`` + ``get_current_state``.
    # The key contract — that ``record_error`` writes to the buffer
    # BEFORE the WS payload is built — is enforced structurally:
    # ``record_error`` is the only writer, and the router reads the
