...
:mod:`hal_service.JogService` and force-stops expired
axes by calling :func:`hal_service.JogService.stop_axis`.

Mirrors the historical 500 ms keep-alive window from the legacy
``backend/routers/jog.py``. The watchdog is started by
:meth:`axis.module.AxisModule.on_load` and
stopped by :meth:`axis.module.AxisModule.on_unload`.
...
async def _loop() -> None:
    ...
    """
    from hal_service.JogService import (
...
def stop_watchdog() -> None:
    ...
    Idempotent. Clears :data:`hal_service.JogService._active_jogs`
    ...
    try:
        from hal_service.JogService import clear_active_jogs
